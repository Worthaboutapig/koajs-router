...
class Router {
  ...
  use(...middleware) {
    ...
    if (
      middleware.length > 0 &&
      typeof middleware[middleware.length - 1] === 'boolean'
    ) {
      matchRoute = middleware.pop();
    } else {
      matchRoute = false;
    }
  ...
  middleware() {
    ...
    const dispatch = (ctx, next) => {
      debug(
        'dispatch middleware for router %s: %s %s',
        this.opts.name,
        ctx.method,
        ctx.path
      );
      ...
      const matchedLayers = matched.pathAndMethod.filter(
        (ml) => ml.opts.matchRoute
      );
      ...
      let layers;
      if (router.exclusive) {
        layers = [mostSpecificLayer];
      } else {
        layers = matched.pathAndMethod.filter(
          (ml) => !ml.opts.matchRoute || ml === mostSpecificLayer
        );
      }

      const layerChain = layers.reduce((memo, layer) => {
        memo.push((ctx, next) => {
  ...
  register(path, methods, middleware, opts = {}) {
    ...
    const route = new Layer(path, methods, middleware, {
      ...
      name: opts.name || middleware.name,
      sensitive: opts.sensitive || this.opts.sensitive || false,
      strict: opts.strict || this.opts.strict || false,
      prefix: opts.prefix || this.opts.prefix || '',
      ignoreCaptures: opts.ignoreCaptures,
      matchRoute: opts.matchRoute !== false
    });
  ...
  match(path, method) {
    ...
    const _method = method || '[ALL VERBS]';

    for (let len = layers.length, i = 0; i < len; i++) {
      layer = layers[i];

      debug(
        'test %s against layer %s: %s %s',
        path,
        layer.name,
        layer.path,
        layer.regexp
      ...
      if (layer.match(path)) {
        debug(
          '1  matched %s:%s against layer %s: %s %s',
          _method,
          path,
          layer.name,
          layer.path,
        ...
        if (layer.methods.length === 0 || layer.methods.includes(method)) {
          debug(
            '2a matched %s:%s against layer %s: %s %s',
            _method,
            path,
            layer.name,
            layer.path,
          ...
          if (layer.methods.length > 0) {
            if (layer.opts.matchRoute) {
              debug(
                '2b route matched %s:%s against layer %s: %s %s',
                _method,
                path,
                layer.name,
                layer.path,
              ...
              debug(
                '2c route bypassed %s:%s against layer %s: %s %s',
                _method,
                path,
                layer.name,
                layer.path,
